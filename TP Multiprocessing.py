...
def socket_windows(success_list, list_ip, min_port, max_port):
...
def ping_windows(success_list, list_ip):
    for ip_str in list_ip:
        try:
            output = subprocess.check_output(f"ping -n 1 -w 1 {ip_str}", shell=True, universal_newlines=True)
            print(f"{ip_str} ONLINE")
            success_list.append(ip_str)
        except:
            print(f"{ip_str} OFFLINE")

def socket_linux(success_list, list_ip, min_port, max_port):
    for ip_str in list_ip:
        for port in range(min_port, max_port):
            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            s.settimeout(1)
...
def ping_linux(success_list, list_ip):
    for ip_str in list_ip:
        try:
            subprocess.check_output(["ping", "-n", "-W1", "-i", "0.2", "-c", "1", ip_str])
            print(f"{ip_str} ONLINE")
            success_list.append(ip_str)
        except:
            print(f"{ip_str} OFFLINE")
...
List of IPs address :\n")
    ...
    if "-p" in argv:
        h1 = multiprocessing.Process(target=ping_linux, args=[success_list, list_ip[:mid]], daemon=True)
        h2 = multiprocessing.Process(target=ping_linux, args=[success_list, list_ip[mid:]], daemon=True)
        h1.start()
        h2.start()
        
    elif "-s" in argv:
        h1 = multiprocessing.Process(target=socket_linux, args=[success_list, list_ip[:mid], min_port, max_port], daemon=True)
        h2 = multiprocessing.Process(target=socket_linux, args=[success_list, list_ip[mid:], min_port, max_port], daemon=True)
        h1.start()
        h2.start()
...
List of IPs address :\n")
        ...
        if "-p" in argv:
            ...
            h1 = threading.Thread(target=ping_windows, args=[success_list, list_ip[:mid]], daemon=True)
            h2 = threading.Thread(target=ping_windows, args=[success_list, list_ip[mid:]], daemon=True)
            h1.start()
            h2.start()
        
        elif "-s" in argv:
            h1 = threading.Thread(target=socket_windows, args=[success_list, list_ip[:mid], min_port, max_port], daemon=True)
            h2 = threading.Thread(target=socket_windows, args=[success_list, list_ip[mid:], min_port, max_port], daemon=True)
            h1.start()
            h2.start()
